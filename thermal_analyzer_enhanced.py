...
With Heat Transfer Coefficient Calculation
"""
...
class SimplePredictor:
    """Enhanced predictor for thermal properties"""
    ...
    def train_model(self, data_list):
        ...
        for record in data_list:
            ...
            self.training_data.append({
                'features': features,
                'target_temp': record['insulation_surface_temp'],
                'target_h_conv': record.get('calculated_h_conv', record['convection_coefficient'])
            })
    ...
    def predict_temperature(self, equipment_temp, cross_section_area, 
        ...
        """Predict insulation surface temperature"""
        ...
        for sample in self.training_data:
            ...
            targets.append(sample['target_temp'])
        
        total_weight = sum(weights)
        prediction = sum(w * t for w, t in zip(weights, targets)) / total_weight
        
        return prediction
    
    def predict_convection_coefficient(self, equipment_temp, insulation_temp, 
                                     cross_section_area, geometry_type, insulation_type):
        """Predict convection coefficient"""
        ...
        # Calculate weighted prediction for h_conv
        weights = []
        targets = []
        
        for sample in self.training_data:
            sample_features = [
                sample['features'][0],  # equipment_temp
                sample['target_temp'],  # insulation_temp
                sample['features'][1],  # cross_section_area
                sample['features'][3],  # geometry_code
                sample['features'][4]   # insulation_code
            ]
            
            distance = 0
            for i, feature in enumerate(input_features):
                distance += (feature - sample_features[i]) ** 2
            
            distance = distance ** 0.5
            weight = 1 / (distance + 0.001)
            
            weights.append(weight)
            targets.append(sample['target_h_conv'])
...
class EnhancedThermalAnalyzer:
    """Enhanced thermal analyzer with heat transfer calculations"""
    ...
    def predict_properties(self, equipment_temp, geometry_type, insulation_type, 
        ...
        """Predict thermal properties for new configuration"""
        ...
        try:
            ...
            # Predict insulation temperature
...
def main():
    """Main enhanced program function"""
    ...
    while True:
        ...
        print("5. Complete thermal analysis prediction")
        ...
        if choice == '1':
            ...
            try:
                print("\nComplete thermal analysis prediction:")
                geometry = input("Geometry type (pipe/sphere/cube/surface): ").strip()
                eq_temp = float(input("Equipment surface temperature (°C): "))
                ins_type = input("Insulation type (polyurethane/foam/glass wool): ").strip()
                area = float(input("Cross-section area (m²): "))
            ...
            print("Enhanced program closed. Good luck!")
            break
